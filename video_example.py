...
from models.model_7 import PrepNetwork, HidingNetwork, RevealNetwork
...
prep_net.load_state_dict(remove_module_prefix(torch.load("models/model_7/prep_net.pth", map_location=device)))
hide_net.load_state_dict(remove_module_prefix(torch.load("models/model_7/hide_net.pth", map_location=device)))
reveal_net.load_state_dict(remove_module_prefix(torch.load("models/model_7/reveal_net.pth", map_location=device)))
...
# Video to Frames
video_file = "video_processing/input_video.mp4"  # Change this to your video file path
extract_frames(video_file)
# Define transforms
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Resize((256, 256)) # 128, 128 for model_5 and before
])
# Create dataset and dataloader
dataset = VideoFramesDataset("video_processing/frames", transform=transform)
...
# Process image
image_file = "video_processing/watermark.jpg" 
secret = convert_image_to_tensor(image_file, transform).unsqueeze(0).repeat(16, 1, 1, 1)
